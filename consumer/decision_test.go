...
// Feature: fifo-scheduled-retry, Property 4: Retry-versus-DLQ decision follows the threshold
func TestScheduledRetryVersusDLQDecision(t *testing.T) {
	log := discardLogger()
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		d.processScheduled(context.Background(), msg, errors.New("handler failure"))
		...
		assert.Equal(t, 1, schedules+sends)
	})
}
